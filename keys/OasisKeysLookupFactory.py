...
class OasisKeysLookupFactory(object):
    ...
    def write_oasis_keys_file(cls, records, output_file_path):
        """
        Writes an Oasis keys file from an iterable of lookup records.
        """
    ...
    def write_list_keys_file(cls, records, output_file_path):
        """
        Writes the lookup records as a simple list to file.
        """
        n = 0
        with io.open(output_file_path, 'w', encoding='utf-8') as f:
            for r in records:
    ...
    def get_keys(
        ...
        """
        Generates keys lookup records (JSON) for the given model and supplier -
    ...
    def save_keys(
        ...
        """
        Writes the keys lookup records generated by the lookup service for the
